...
server.post("/episodes/", function(req, res, next) {
  const error = validateEpisode(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new episodes.
    next();
  }
});
...
function validateEpisode(episode) {
  if (!episode.title) return "Title is required.";
  if (!episode.authorId) return "Author is required.";
  if (!episode.category) return "Category is required.";
  return "";
}
